...
export default function App() {
  ...
  const resetForm = () => { setStep(0); setPseudo(""); setPseudoConfirmed(false); setAnswers({}); setView("form"); };
                ...
                {!pseudoConfirmed ? (
                  <div style={{ animation: "slideUp 0.4s ease both" }}>
                    ...
                    <input autoFocus value={pseudo} onChange={(e) => setPseudo(e.target.value)}
                      onKeyDown={(e) => { if (e.key === "Enter" && pseudo.trim().length > 1) setPseudoConfirmed(true); }}
                    ...
                    />
                    <button onClick={() => { if (pseudo.trim().length > 1) setPseudoConfirmed(true); }} disabled={pseudo.trim().length < 2}
                  ...
                  <QuestionStep key={step} q={QUESTIONS[step]}
                    ...
                    onPrev={() => { if (step > 0) setStep(step - 1); else { setPseudo(""); setPseudoConfirmed(false); } }}
